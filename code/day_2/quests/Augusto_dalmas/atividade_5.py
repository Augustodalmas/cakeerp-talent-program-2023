"""
5 - Faça um programa com uma função chamada somaImposto. A função possui dois parâmetros formais:
taxaImposto, que é a quantia de imposto sobre vendas expressa em porcentagem e custo,
que é o custo de um item antes do imposto. A função “altera” o valor de custo para incluir o imposto sobre vendas
"""

def somaImposto(taxa, valor):
    custo_final = valor * (taxa/100)
    return custo_final + valor


print("Seu valor de produto com Imposto é: ", somaImposto(int(input("Qual a taxa do produto em %? ")), int(input("Qual o custo do produto? "))))